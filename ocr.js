...
function ocrLine(lineNum) {

}

function scanLine(data, lineNum) {
  ...
  let _offsetTop = ocrInfo.offsetTop + (ocrInfo.offsetTop + ocrInfo.lineDistance) * lineNum

  for (var y = _offsetTop; y < _offsetTop + ocrInfo.fontHeight; y++) {
  ...
  return Object.keys(ocrdb).map((k) => ocrdb[k].join(''))
}

pngDecode(pngName).then(data => {
    console.log(scanLine(data, 0))
...
//
//
//     splitArray(_study,60).forEach(item => {
//       console.log(item.join(''))
//     })
//
//  })
